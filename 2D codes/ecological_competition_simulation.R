...
plot_walkers <- function(df_walkers, step_num, pop_A, pop_B, domain_size,
                         ...
                         title_main = "2D Lévy Flight Motion with Two Competing Species") {
  
  subtitle_text <- sprintf(
    "STEP: %d | Population A: %d | Population B: %d | R: %.2f\nr_b0: %.2f | r_d0: %.2f | α: %.2f | β: %.2f | κ_b: %.2f | κ_d: %.2f\nmu_A: %.2f | mu_B: %.2f | l_min: %.4f\nγ_fast: %.2f | γ_low: %.2f | R_MIPS: %.2f",
    step_num, pop_A, pop_B, R,
    r_b0, r_d0, alfa, beta, kappa_b, kappa_d,
    mu_A, mu_B, l_min,
    gamma_fast, gamma_slow, R_MIPS
  )
...
warming_steps <- 500           # Initial steps before equilibrium check
